...
from messages_system.utils import get_user_model, get_username_field
...
class ListUserField(forms.Field):
    widget = CommaSeparatedUserInput

    def __init__(self, *args, **kwargs):
        recipient_filter = kwargs.pop('recipient_filter', None)
        self._recipient_filter = recipient_filter
        super(ListUserField, self).__init__(*args, **kwargs)

    # def get_username_field():
    #     return "username"

    def clean(self, value):
        super(ListUserField, self).clean(value)
        if not value:
            return ''
